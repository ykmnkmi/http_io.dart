...
class WebSocketMessageCollector {
  ...
  WebSocketMessageCollector(Stream stream, [List<int>? this.expectedMessage]) {
    stream.listen(onMessageData, onDone: onClosed, onError: onError);
  }

  void onMessageData(Object? buffer) {
    if (buffer is String) {
      buffer = utf8.encode(buffer);
    }
    Expect.listEquals(expectedMessage!, buffer as List);
    messageCount++;
    data = buffer;
  }
