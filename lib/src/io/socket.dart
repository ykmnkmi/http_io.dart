...
abstract interface class RawServerSocket implements Stream<RawSocket> {
  ...
  external static Future<RawServerSocket> bind(Object address, int port,
...
abstract interface class ServerSocket implements ServerSocketBase<Socket> {
  ...
  static Future<ServerSocket> bind(Object address, int port,
      {int backlog = 0, bool v6Only = false, bool shared = false}) {
    return ServerSocket._bind(address, port,
        backlog: backlog, v6Only: v6Only, shared: shared);
  }

  external static Future<ServerSocket> _bind(Object address, int port,
      {int backlog = 0, bool v6Only = false, bool shared = false});
...
final class RawSocketOption {
  ...
  factory RawSocketOption.fromInt(int level, int option, int value) {
    ...
    var buffer = ByteData.view(list.buffer, list.offsetInBytes);
...
abstract interface class RawSocket implements Stream<RawSocketEvent> {
  ...
  external static Future<RawSocket> connect(Object host, int port,
  ...
  external static Future<ConnectionTask<RawSocket>> startConnect(
      ...
      {Object? sourceAddress, int sourcePort = 0});
...
abstract interface class Socket implements Stream<Uint8List>, IOSink {
  ...
  /// longer needed.
  static Future<ConnectionTask<Socket>> startConnect(Object host, int port,
      {Object? sourceAddress, int sourcePort = 0}) {
    return Socket._startConnect(host, port,
        sourceAddress: sourceAddress, sourcePort: sourcePort);
  }

  external static Future<Socket> _connect(Object host, int port,
      {Object? sourceAddress, int sourcePort = 0, Duration? timeout});
...
abstract interface class RawDatagramSocket extends Stream<RawSocketEvent> {
  ...
  external static Future<RawDatagramSocket> bind(Object host, int port,
