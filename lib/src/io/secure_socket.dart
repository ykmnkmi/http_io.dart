...
abstract interface class SecureSocket implements Socket {
  ...
  static Future<SecureSocket> connect(Object host, int port,
  ...
  static Future<ConnectionTask<SecureSocket>> startConnect(
    ...
    return RawSecureSocket.startConnect(host, port,
            ...
            supportedProtocols: supportedProtocols)
        .then((rawState) {
      Future<SecureSocket> socket = rawState.socket.then(SecureSocket._);
  ...
  static Future<SecureSocket> secure(Socket socket,
    ...
    return socket._detachRaw().then<RawSecureSocket>((detachedRaw) {
      return RawSecureSocket.secure(detachedRaw[0] as RawSocket,
          ...
          keyLog: keyLog,
          supportedProtocols: supportedProtocols);
    }).then<SecureSocket>(SecureSocket._);
  }
...
abstract interface class RawSecureSocket implements RawSocket {
  ...
  static Future<RawSecureSocket> connect(Object host, int port,
      {SecurityContext? context,
      bool Function(X509Certificate certificate)? onBadCertificate,
      void Function(String line)? keyLog,
      List<String>? supportedProtocols,
  ...
  static Future<RawSecureSocket> secure(RawSocket socket,
    ...
    return _RawSecureSocket.connect(
        host ?? socket.address.host, socket.port, false, socket,
...
class _RawSecureSocket extends Stream<RawSocketEvent>
  ...
  final Completer<RawSecureSocket> _closeCompleter =
      Completer<RawSecureSocket>();
  ...
  static Future<_RawSecureSocket> connect(
      Object /*String|InternetAddress*/ host,
      int requestedPort,
      bool isServer,
      RawSocket socket,
      {SecurityContext? context,
    ...
    InternetAddress address =
        InternetAddress._cloneWithNewHost(socket.address, host as String);
  ...
  _RawSecureSocket(
    ...
    var secureFilter = _secureFilter!;
    secureFilter.init();
    secureFilter
  ...
  static void _verifyFields(Object host, int requestedPort,
  ...
  void _reportError(Object e, [StackTrace? stackTrace]) {
  ...
  Future<void> _closeHandler() async {
    if (_status == connectedStatus) {
      if (_closedRead) {
        return;
      }
      ...
      if (_filterStatus.readEmpty) {
        _reportError(
            HandshakeException('Connection terminated during handshake'), null);
      }
    }
  }
  ...
  List<int>? _readSocketOrBufferedData(int bytes) {
    var bufferedData = _bufferedData;
  ...
  Future<_FilterStatus> _pushAllFilterStages() async {
    ...
    List<dynamic> args = List<dynamic>.filled(2 + bufferCount * 2, null);
    ...
    if (response.length == 2) {
      if (wasInHandshake) {
        ...
        _reportError(
            HandshakeException('${response[1]} error ${response[0]}'), null);
      } else {
