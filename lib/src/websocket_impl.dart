...
class _WebSocketProtocolTransformer
    extends StreamTransformerBase<List<int>, Object>
  ...
  Stream<Object> bind(Stream<List<int>> stream) {
    return Stream.eventTransformed(stream, (EventSink<Object> eventSink) {
      if (_eventSink != null) {
  ...
  void _unmask(int index, int length, Uint8List buffer) {
    ...
    if (length >= BLOCK_SIZE) {
      ...
      int startOffset = BLOCK_SIZE - (index & 15);
      ...
      if (blockCount > 0) {
        ...
        int bytes = blockCount * BLOCK_SIZE;
        index += bytes;
        length -= bytes;
      }
    }
...
typedef _ProtocolSelector = Object Function(List<String> protocols);
...
class _WebSocketTransformerImpl
  ...
  static Future<WebSocket> _upgrade(HttpRequest request,
    ...
    Future<WebSocket> upgrade(String? protocol) {
      ...
      _WebSocketPerMessageDeflate? deflate =
          _negotiateCompression(request, response, compression);
    ...
    if (protocols != null && protocolSelector != null) {
      ...
      List<String> tokenizedProtocols =
          _tokenizeFieldValue(protocols.join(', '));
      return Future<String>(
              () => protocolSelector(tokenizedProtocols) as String)
  ...
  static _WebSocketPerMessageDeflate? _negotiateCompression(
    ...
    HeaderValue hv = HeaderValue.parse(extensionHeader, valueSeparator: ',');

    if (compression.enabled && hv.value == _WebSocketImpl.PER_MESSAGE_DEFLATE) {
      _CompressionMaxWindowBits info = compression._createHeader(hv);
...
class _WebSocketPerMessageDeflate {
  ...
  Uint8List processIncomingMessage(List<int> msg) {
    ...
    List<int> data = <int>[...msg, 0x00, 0x00, 0xff, 0xff];
...
class _WebSocketOutgoingTransformer
  ...
  static Iterable<List<int>> createFrame(
    ...
    // Set FIN and opcode.
    int hoc = _WebSocketProtocolTransformer.FIN |
...
class _WebSocketConsumer implements StreamConsumer<Object> {
  final _WebSocketImpl webSocket;
  ...
  StreamController<Object> _ensureController() {
    ...
    controller = _controller = StreamController<Object>(
        sync: true,
        onPause: _onPause,
        onResume: _onResume,
        onCancel: _onListen);
...
class _WebSocketImpl extends Stream<Object>
    with _ServiceObject
  ...
  _WebSocketImpl._fromSocket(
    ...
    _WebSocketProtocolTransformer transformer =
        _WebSocketProtocolTransformer(_serverSide, deflate);
    StreamSubscription<Object> subscription =
